...
### _Activitate: Urmărește „Cum funcționează Bitcoin sub capotă”_
...
> „Ceea ce ne oferă Bitcoin este o promisiune fermă: programul se va executa exact așa cum este specificat.”    
 **$\color[RGB]{250,175,64}Andreas \ M. \ Antonopoulos$**
...
Așadar, principalul avantaj al criptografiei cu cheie publică față de cea privată este că permite comunicarea securizată fără ca expeditorul și destinatarul să fie nevoiți să partajeze mai întâi o cheie secretă (sau o altă metodă de criptare precum cifrul Pigpen), care ar putea fi interceptată de o terță parte.
...
- 💡 **Criptografie cu cheie publică (pentru fiecare tranzacție între doi utilizatori):** <br> <br> Fiecare utilizator are două chei: o **$\color[RGB]{255,166,21} cheie \ privată$**, care este **$\color[RGB]{255,166,21} păstrată \ secretă$**, și o **$\color[RGB]{104,59,147} cheie \ publică$** care poate fi **$\color[RGB]{104,59,147} partajată \ cu \ alții$**. <br> <br> **$\color[RGB]{255,166,21} Cheia \ privată$** servește ca formă de identificare și dovadă a proprietății, confirmând: **„Această adresă îmi aparține și am control asupra ei.”** <br> <br> Se creează **$\color[RGB]{31,110,199} semnături \ digitale$** pentru a identifica tranzacțiile unice.

<div><p align="center"><img src="Images/19.Chapter-9/15.Digital-signature-v1.png" width="420"></div>

<br/>

- ⭐ Tranzacțiile Bitcoin implică transferul unei anumite sume de bitcoini direct în contul altei persoane.

- ⭐ doar adevăratul deținător al bitcoinilor are controlul de a trimite banii către altcineva. Acest lucru asigură că proprietatea este protejată de actori rău intenționați.

- ⭐ Ca măsură suplimentară de protecție, fiecare tranzacție pe care o trimiți în Bitcoin primește automat o **$\color[RGB]{31,110,199} SEMNĂTURĂ \ UNICĂ$**. Această **$\color[RGB]{31,110,199} semnătură \ unică$** este susținută de o tehnologie rezistentă la manipulare care ajută rețeaua să verifice că adevăratul proprietar al bitcoinilor, și nu altcineva, i-a trimis.
...
1. ⭐ **Crearea tranzacției:** <br>
    Un utilizator inițiază o tranzacție bitcoin specificând detalii precum adresa destinatarului și suma de bitcoini care urmează să fie trimisă.
...
3. ⭐ **Transmiterea tranzacției:** <br>
    Tranzacția semnată este transmisă rețelei Bitcoin, indicând intenția de a transfera proprietatea bitcoinilor de la expeditor la destinatar.
...
5. ⭐ **Confirmarea pe rețeaua Bitcoin:** <br>
    Dacă verificarea are succes, tranzacția va fi adăugată în registru, care servește ca o evidență sigură și transparentă a tuturor tranzacțiilor. Odată confirmată, proprietatea bitcoinilor este transferată oficial de la expeditor la destinatar.
...
- 💡 O **funcție** este ca o mașină care primește niște informații și le transformă în ceva nou. Informația pe care o dai funcției se numește **$\color[RGB]{255,166,21} intrare$**. Noua informație creată de funcție se numește **$\color[RGB]{31,110,199} rezultat$**. Funcțiile ajută calculatoarele să îndeplinească sarcini și să rezolve probleme.
...
Această rețetă este o funcție care ia ingredientele ca **$\color[RGB]{255,166,21} intrare$** și generează salata amestecată ca **$\color[RGB]{31,110,199} rezultat$**.
...
O funcție hash este un tip de funcție unidirecțională care primește o intrare (numită „mesaj” sau „date”) și o convertește într-o reprezentare numerică numită „hash”. **$\color[RGB]{31,110,199}  output$** hash este unic pentru datele de intrare, astfel încât chiar și o mică modificare a **$\color[RGB]{87,88,91} input$** va duce la un hash complet diferit.
...
**$\color[RGB]{31,110,199}  output$**, sau hash-ul, are întotdeauna aceeași lungime, indiferent cât de lungă a fost informația originală.

Bitcoin folosește câteva tipuri specifice de funcții hash numite **SHA-256** și **RIPEMD160**. Câteva exemple sunt mai jos:

- ⭐ Observă că o mică schimbare în a doua intrare schimbă complet rezultatul față de primul exemplu.

- ⭐ A treia intrare este un fișier uriaș, dar rezultatul are aceeași lungime fixă ca și celelalte două.
